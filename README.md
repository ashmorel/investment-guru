...
Enhancement 1 (multi-user + encryption + admin) complete — open self-service registration with strict per-user isolation; financially-sensitive data (holdings quantities/costs, ORSO allocations, Guru report payloads, chat, profile free-text, position notes) is **encrypted at rest** with a server-held Fernet key (`DATA_ENCRYPTION_KEY`, distinct from the committed dev key, refused in prod); an email-allowlisted admin role (`ADMIN_EMAILS`) + admin-area shell; a per-user daily LLM budget (429 `budget_exhausted`); and an opt-in daily digest (the scheduler iterates opted-in users). See `docs/superpowers/specs/2026-07-09-multiuser-encryption-design.md`.

ORSO data-entry + advice (migration 0009) complete — get real ORSO holdings in fast: upload a **CSV**, a **statement screenshot** (read via the Guru vision model), or enter manually — all three produce a draft you **review and edit** before one transactional, switch-logged save. Funds carry a **native currency** and the overview totals render in a **switchable display currency** (each fund FX-converted; FX outages degrade per-fund, never error). A statement's units + value derive an implied fund price, so valuation works without a live feed (the built-in HSBC price feed is WMFS-only and doesn't cover the Local Staff DC Scheme). Fund search over your own menu, and the Guru's ORSO advice now speaks to your **retirement-goal gap** — suggesting concrete contribution/allocation changes, regenerable on demand. See `docs/superpowers/specs/2026-07-11-orso-data-entry-advice-design.md`.

Enhancement 2 (multi-provider LLM + admin config, migration 0010) complete — the Guru runs on **Anthropic, OpenAI, or Google Gemini**, chosen from an **admin-only panel** (`/admin`): pick the provider, the advice + scan model ids, and paste the API key. The key is **encrypted at rest** and never returned to the browser (the panel shows only "configured"); switching provider **takes effect immediately, no redeploy**. Optional per-million price fields keep the daily budget accurate for any model, and a **Test connection** button validates the key before saving. Only `lee_ashmore@hotmail.co.uk` can change it. See `docs/superpowers/specs/2026-07-12-multiprovider-llm-admin-design.md`.

Enhancement 3 (dashboard/stock-news UX, migration 0011) complete — a readable **News panel** on the dashboard (and a per-position list): your holdings' headlines, **de-duplicated** and grouped by stock, ranked most-active first, each with source, relative time, and a link. Headlines **auto-refresh** when the cache is stale (plus a manual Refresh) and always render. A per-stock **Summarize** button generates an on-demand **Guru summary** — a short "what's happening" write-up + a color-coded sentiment tag + key points — that's **saved and regenerable**, budget-gated, and runs on the cheap scan model. Next: enhancement 4 (sector grouping). See `docs/superpowers/specs/2026-07-12-dashboard-news-ux-design.md`.
